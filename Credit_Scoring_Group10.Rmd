...
# Set charachter variables which are not a factor as factor so the glm function can work with them
...
In this excercise we review the data pre-processing steps that we carried out before training our model.
...
* Outliers:
  ...
  * The Outliers could be removed by applying a function and substituting the outliers by the 5th or 95th quantile value. This could have a chance to improve the predictive performance of the model.
* Categorial Variables
  * We converted character variables to factors in Excercise 2.2, to assist the model in the training process.
...
* Extended Data Collection
  * The bank could think about collecting more data to improve the model. For example the bank could collect more data about the customers, like their age, their job, their credit score, their credit history, their debt-to-income ratio, the interest rate etc. While these attributes could improve the prediction a lot, they should be handled with care and be anonymised for data protection reasons.
...
In the next step, we apply the substitute_outliers function to the numeric variables in the dataset.

```{r}
balanced_numeric <- map_df(balanced_data[,-c(3,4,6,7,14,17)], substitute_outliers)
cols <- balanced_data[,c(3,4,6,7,14,17)]
...
The plot suggests that the application_type variables is not important to the model, so we remove it.
```{r}
...
The accuracy of the model is now at 64.21%, which is not much more than initially (63,98%).
The AUC value is now 68.71%, which is even lower than initially (68.9%).

This means that the improvements, like removing the outliers and the application_type variable, did not improve the model.
We think that the reason for this is that the dataset is maybe not big enough to improve the model by removing outliers and variables.
