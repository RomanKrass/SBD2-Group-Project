...
libraries = c("readr", "tidyverse", "dlookr", "ggplot2", "Hmisc", "reshape2", "plotly", "caret", "ROCR")
...
plot_numeric_by_target <- function(data, target) {
  ...
  for (var in names(data_numeric)) {
    ...
    print(
      ggplot(data, aes_string(x = as.factor(target), y = var, fill = as.factor(target))) +
      geom_boxplot(fill = custom_fill, color = custom_color) +
...
plot_categorical_associations <- function(data, target) {
  ...
  for(var in names(data_categorical)) {
    ...
    plot <- ggplot(data, aes_string(x = var)) +
      geom_bar(position = "fill", fill = custom_fill, color = custom_color) +
...
# Apply the function to the loan dataset
plot_categorical_associations(data, "Status")
```
...
# Training Sample
data.train <- balanced_data[div,] # 70% here

# Test Sample
data.test <- balanced_data[-div,] # rest of the 30% data goes here
